...
function generate_smilies($mode, $forum_id, $acp_announce = false)
{
	...
	if ($mode == 'window')
	{
		...
		$template->assign_var('PAGINATION',
		    generate_pagination(( $acp_announce ) ? append_sid("{$phpbb_root_path}posting.$phpEx", 'mode=smilies&announce=1') : append_sid("{$phpbb_root_path}posting.$phpEx", 'mode=smilies&amp;f=' . $forum_id),
