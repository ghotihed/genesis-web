...
$help = array(
	...
	array(
		0 => '--',
		1 => 'Genesis sci fi club forum'
	),
	array(
		0 => 'What are the terms and conditions?',
		1 => '<UL>
  <LI>The Genesis sci fi club forum is free and may be used by club members and non-members alike.</LI>
  <LI>You must be at least 12 years old or be supervised by an adult.  Genesis sci fi club is not aimed at children or families.</LI>
  <LI>You must follow basic netiquette (don\'t be offensive, no flooding or spamming, enclose spoilers in spoiler tags etc).</LI>
  <LI>Remember that the forum can be viewed by anyone.  Just avoid subjects that would disturb a typical, British teenager, which is quite hard to do.</LI>
  <LI>Messages must be relevant to the subject of the forum or topic.</LI>
  <LI>Message subject text may be edited by moderators without warning to avoid confusion or inaccuracy but these will be marked by \'[MOD]\' text.</LI>
  <LI>Do not post pirated material or refer to piracy sites.</LI>
  <LI>You may only have one account.  A couple may share an account but it must be obvious that it\'s shared and they are responsible for each other\'s online conduct.  Group accounts are not allowed.</LI>
  <LI>You must maintain a valid email address.  An account with an invalid address will be disabled to prevent further \'bouncing\' emails.</LI>
  <LI>Accounts of non-club members that are inactive will be deleted after 3 months.</LI>
  <LI>You must keep your password secret.  An account that appears to hacked will be disabled immediately.</LI>
  <LI>Administrative requests should be posted in the Help, Bugs and Suggestions forum or, if this is not possible, emailed to webmaster@genesis-sf.org.uk</LI>
</UL>'
	),
	array(
		0 => 'What is the privacy policy?',
		1 => '<UL>
	<LI>The Genesis sci fi forum requires email addresses for forum administration and any notifications you choose to receive.  They will not be revealed to third parties, even when one member emails another, without your explicit permission.</LI>
     <LI>You may enter other information with your account, such as real name or location, which is publicly displayed. </LI>
     <LI>Anything posted on this forum, besides email addresses and administration messages, is assumed to be public domain and can be reproduced anywhere else, especially on this website, unless explicitly forbidden by the author.  The author will be acknowledged.</LI>
</UL>'
	)
);
